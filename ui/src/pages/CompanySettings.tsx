...
import { Settings, Check, Download, Upload, Package, Loader2 } from "lucide-react";
...
export function CompanySettings() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {/* Packs */}
      <PacksSection selectedCompanyId={selectedCompanyId} />
...
function PacksSection({ selectedCompanyId }: { selectedCompanyId?: string | null }) {
  const { pushToast } = useToastActions();
  ...
  if (packs.length === 0) {
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground text-center">
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {(packs as any[]).map((pack: any) => (
          <div key={pack.slug} className="rounded-md border border-border px-4 py-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {pack.tagline && (
                  <p className="text-xs font-medium mt-1">{pack.tagline}</p>
                )}
                {pack.value_proposition && pack.value_proposition.length > 0 && (
                  <ul className="text-xs text-muted-foreground mt-2 space-y-1">
                    {(pack.value_proposition as string[]).map((prop: string, idx: number) => (
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
                  {pack.estimated_setup_minutes && <span>⏱ ~{pack.estimated_setup_minutes} min</span>}

...
import { skillPerformanceRoutes } from "./routes/skill-performance.js";
...
import { taskDagRoutes } from "./routes/task-dag.js";
import { skillVersionRoutes } from "./routes/skill-version.js";
...
import { instanceApiKeysRoutes } from "./routes/instance-api-keys.js";
import { qualityGateRoutes } from "./routes/quality-gates.js";
import { integrationRoutes } from "./routes/integrations.js";
import { instanceLlmModelsRoutes } from "./routes/instance-llm-models.js";
...
export async function createApp(
  ...
  api.use(skillPerformanceRoutes(db));
  api.use(taskInlineEditRoutes(db));
  ...
  api.use(membersRoutes(db));
  api.use(pushRoutes(db));
  api.use(billingRoutes(db));
  api.use(adminSkillRoutes(db));
  api.use(gdprRoutes(db));
  api.use(taskDagRoutes(db));
  api.use(skillVersionRoutes(db));
  api.use(batchRoutes(db));
  api.use(multimodalRoutes(db));
  api.use(mcpServerRoutes(db));
  api.use(publicApiRoutes(db));
  api.use(a2aRoutes(db));
  api.use(extensionRoutes(db));
  api.use(instanceApiKeysRoutes(db));
  api.use(qualityGateRoutes(db));
  api.use(integrationRoutes(db));
  api.use(instanceLlmModelsRoutes(db));
  api.use(adminPlanRoutes(db));

...
// ── Card generators (injected for testability) ────────────────────────────────
 ...
 */
export async function generateIntelligenceCards(
  db: Db,
  companyId: string,
  ...
  const candidatesNested = await Promise.all(
    generators.map((gen) => gen(db, companyId)),
  );

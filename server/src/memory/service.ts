...
export async function searchMemory(
  ...
  for (const chunk of relevant) {
    if (chunksUsed >= params.maxChunks) break;
    const chunkText = `[Memory] ${chunk.title}: ${chunk.content}\n`;
    if (estimateTokens(text + chunkText) > params.maxTokens) break;
    text += chunkText;
    chunksUsed++;
  }
...
export async function buildContactProfile(
  ...
  const [contact] = await db
    ...
    .where(
      and(
        ...
        ilike(contacts.fullName, `%${contactName}%`),
      ),
    )
    .limit(1);
  ...
  if (contact.organisation) lines.push(`Organisation: ${contact.organisation}`);
  if (contact.email)        lines.push(`Email: ${contact.email}`);
  if (contact.notes)        lines.push(`Background: ${contact.notes}`);
 ...
 */
export async function injectContactProfile(
  db: Db,
  companyId: string,
  query: string,
): Promise<string | null> {

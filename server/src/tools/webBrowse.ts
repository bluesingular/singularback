 ...
 */
async function getAgentWebWhitelist(_db: Db, _agentId: string): Promise<string[]> {
  return [];
}
 ...
 */
async function flagForGDPRReview(
  companyId: string,
  url: string,
  contentLength: number,
): Promise<void> {
  logger.warn(
    { companyId, url, contentLength },
    "web-browse: GDPR flag — content may contain personal data, manual review required",
  );
}
...
function createFirecrawl(): FirecrawlApp {
  ...
  if (context.skill.webScope === "restricted") {
    ...
    if (!allowed.includes(domain)) {
      throw new ForbiddenError(
        `Domain ${domain} not in this agent's web whitelist`,
      );
    }
  }

  const startTime = Date.now();
  let result: WebBrowseOutput | undefined;
  ...
  try {
    ...
    if (input.mode === "scrape") {
      ...
      result = {
        ...
        metadata: {
          date: scraped.metadata?.publishedTime as string | undefined,
          author: scraped.metadata?.author as string | undefined,
          canonical: scraped.metadata?.sourceURL as string | undefined,
        },
    ...
    logger.warn(
      { err, url: input.url },
      "web-browse: Firecrawl failed, falling back to Jina Reader",
    );

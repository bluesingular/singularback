  ...
  try {
    ...
    switch (source) {
      ...
      case "stripe":
        ...
        received = (req.headers["stripe-signature"] as string)?.split(",")
  ...
  router.post("/:companyId/:agentSlug", async (req, res) => {
    ...
    try {
      ...
      if (integration?.webhookSecret) {
        ...
        if (!valid) {
          logger.warn(
            { companyId, source, agentSlug },
            "webhook: invalid HMAC signature — discarding event",
          );
          return;
        }
      }
      ...
      if (!agent) {
        logger.warn(
          { companyId, agentSlug, eventId: event?.id },
          "webhook: no agent found for slug — event stored but not dispatched",
        );
        return;
      }
      ...
      logger.info(
        { companyId, agentSlug, agentId: agent.id, source, eventId: event?.id },
        "webhook: event queued",
      );
    } catch (err) {
      // Errors here are silent to the caller (already got 200)
      logger.error(
        { err, companyId, agentSlug, source },
        "webhook: failed to process inbound event",
      );
    }
  });

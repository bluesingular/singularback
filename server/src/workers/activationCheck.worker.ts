...
import { companies, issues, contactEvents, intelligenceCards, activationMoments } from "@paperclipai/db";
import type { Db } from "@paperclipai/db";
import { redisConnectionBlocking } from "../queue/redis.js";
import {
  STANDARD_TRIGGERS,
  getFiredTriggers,
  recordActivationFired,
  type TriggerKey,
  type ActivationState,
} from "../activation/sequence.js";
import type { ActivationCheckJob } from "../queue/jobs.js";
...
async function resolveActivationState(
  ...
  const [contactRow] = await db
    .select({ total: sql<number>`count(distinct ${contactEvents.contactId})` })
    .from(contactEvents)
...
const ACTIVATION_CARDS: Record<
  TriggerKey,
  { title: string; body: string; urgency: 1 | 2 | 3 | 4 | 5 }
> = {
  day_0_seed: {
...
export function createActivationCheckWorker(db: Db) {
  return new Worker<ActivationCheckJob>(
    "system",
...
let _worker: ReturnType<typeof createActivationCheckWorker> | null = null;

export function initActivationCheckWorker(db: Db) {

...
async function processHeartbeat(job: Job<HeartbeatJob>): Promise<void> {
...
export const heartbeatWorker = new Worker<HeartbeatJob>(
  "agents",
  async (job) => {
    if (job.name !== "heartbeat") return; // this worker only handles heartbeat jobs
...
heartbeatWorker.on("failed", (job, err) => {
  logger.error(
    { agentId: job?.data?.agentId, jobId: job?.id, err },

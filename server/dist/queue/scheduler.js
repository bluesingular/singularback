...
export async function bootstrapScheduler(db) {
    ...
    logger.info({ count: activeCompanies.length }, "scheduler: monthly cost resets registered");
    ...
    for (const agent of activeAgents) {
        ...
        await emit.heartbeat({ agentId: agent.id, companyId: agent.companyId, triggeredBy: "scheduler" }, 0);
        seeded++;
    }
    logger.info({ total: activeAgents.length, seeded, skipped }, "scheduler: heartbeat jobs bootstrapped");
}
/**

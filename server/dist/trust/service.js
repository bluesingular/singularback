...
import { calculateTrustScore, getAutonomyLevel, isDowngrade, nextLevel, } from "./calculator.js";
 ...
 */
export async function recordApproval(db, params) {
    const { companyId, agentId, skillType, skillAutonomyTier, rating, qualityRatingAvg, gatePassRate, schemaPassRate, taskCountWindow, currentStreak, currentLevel, } = params;
    // 1. Compute new score
    ...
    if (triggerProposal) {
        ...
        if (target !== null) {
            if (skillAutonomyTier === "A") {
                ...
                newLevel = target;
                autoActivated = true;
                logger.info({ agentId, skillType, from: currentLevel, to: target }, "trust: Tier B auto-activation");
            }
        }
    }
 ...
 */
export async function checkTrustDowngrade(db, params) {
    ...
    logger.warn({ agentId, skillType, from: currentLevel, to: newLevel, newScore }, "trust: autonomy downgraded");

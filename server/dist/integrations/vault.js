 ...
 */
function getCompanyKey(companyId) {
    const masterKeyHex = process.env.VAULT_MASTER_KEY;
    if (!masterKeyHex) {
        throw new VaultError("VAULT_MASTER_KEY is not configured. Set a 64-character hex string in environment.");
    }
    if (masterKeyHex.length !== 64) {
        throw new VaultError(`VAULT_MASTER_KEY must be exactly 64 hex characters (32 bytes). Got ${masterKeyHex.length}.`);
    }
    const master = Buffer.from(masterKeyHex, "hex");
    // HKDF: info = company_id provides domain separation between companies
    return Buffer.from(hkdfSync("sha256", master, "", `singular:vault:${companyId}`, KEY_LENGTH));
}
/**
 ...
 */
export function encryptCredential(companyId, plaintext) {
 ...
 */
export function decryptCredential(companyId, enc, iv, tag) {
    const key = getCompanyKey(companyId);
    ...
    try {
        ...
        throw new VaultError("Credential decryption failed: auth tag mismatch. The stored credential may be corrupted.");
    }
}
// ── Convenience helpers ───────────────────────────────────────────────────────
/**
 ...
 */
export function encryptJson(companyId, data) {
    return encryptCredential(companyId, JSON.stringify(data));
}
/**
 * Decrypt and parse a JSON blob.
 */
export function decryptJson(companyId, enc, iv, tag) {
    return JSON.parse(decryptCredential(companyId, enc, iv, tag));
}

...
import { integrations, agentIntegrationPermissions, toolCallLog, } from "@paperclipai/db";
import { decryptCredential } from "./vault.js";
// ── Error types ───────────────────────────────────────────────────────────────
export class MCPPermissionError extends Error {
    constructor(agentId, server, permission) {
        super(`Agent ${agentId} lacks permission "${permission}" on integration "${server}". ` +
            `Grant access via Settings → Integrations.`);
        this.name = "MCPPermissionError";
    }
}
export class MCPIntegrationNotFoundError extends Error {
    constructor(companyId, server) {
        super(`Integration "${server}" is not connected for company ${companyId}. ` +
...
export class MCPRouter {
    ...
    // ── Private helpers ─────────────────────────────────────────────────────────
    async checkPermission(agentId, companyId, server, requiredPerm) {
        // Find the integration first to get its id
        const [integration] = await this.db
            .select({ id: integrations.id })
            .from(integrations)
            .where(and(eq(integrations.companyId, companyId), eq(integrations.type, server)))
            .limit(1);
        if (!integration) {
        ...
        const [perm] = await this.db
            ...
            .where(and(eq(agentIntegrationPermissions.agentId, agentId), eq(agentIntegrationPermissions.integrationId, integration.id)))
            .limit(1);
        if (!perm || !perm.permissions.includes(requiredPerm)) {
            throw new MCPPermissionError(agentId, server, requiredPerm);
        }
    }
    async getIntegration(companyId, server) {
    ...
    resolveAccessToken(companyId, integration) {
        if (integration.oauthAccessTokenEnc) {
            ...
            return decryptCredential(companyId, integration.credentialsEnc, integration.credentialsIv, integration.credentialsTag);
        }
     ...
     */
    async executeMCPCall(server, tool, args, _accessToken) {
        ...
        throw new Error(`MCP adapter for "${server}/${tool}" is not yet implemented. ` +
            `Add it in server/src/integrations/adapters/${server}.ts`);
    }
    async scheduleTokenRefresh(companyId, server) {

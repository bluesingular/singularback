 ...
 */
export function translateRemainingTasks(tasksRemaining, translations) {
    if (tasksRemaining <= 0)
        return ["0 tasks remaining"];
    // Sort by tasksPerUnit descending (most "meaningful" unit first)
    const sorted = [...translations.translations].sort((a, b) => b.tasksPerUnit - a.tasksPerUnit);
 ...
 *   "847 / 2,000 tasks — about 95 more CV batches or 380 client emails this month"
 */
export function formatUsageGauge(tasksUsed, tasksLimit, translations) {
 ...
 */
export function formatOverageWarning(tasksRemaining, translations, overageCostEurPerTask) {
    const primary = translations.translations.sort((a, b) => b.tasksPerUnit - a.tasksPerUnit)[0];
    if (!primary)
        return `You have ${tasksRemaining} tasks left.`;
    ...
    return (`You have ${tasksRemaining} tasks left — about ${units} more ${primary.unitLabel}s.\n` +
        `After that, each ${primary.unitLabel} costs €${costPerUnit} extra.`);
}
//# sourceMappingURL=taskTranslation.js.map
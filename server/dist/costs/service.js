 ...
 */
export function calculateCostMicro(model, inputTokens, outputTokens) {
    const costs = MODEL_COSTS[model];
    if (!costs)
        return 0;
    return Math.round((inputTokens / 1_000_000) * costs.input +
 ...
 */
export async function recordUsage(db, params) {
    const costEurMicro = calculateCostMicro(params.model, params.inputTokens, params.outputTokens);
 ...
 */
export async function checkBudgetBeforeCall(db, companyId) {
    ...
    if (company.tasksUsedMonth >= company.tasksLimitMonth) {
        throw new BudgetExhaustedError("Monthly task limit reached. Upgrade your plan or wait for the next billing cycle.");
    }
 ...
 */
async function checkT3Cap(db, companyId) {
    const [company] = await db
        .select({
        tasksUsedMonth: companies.tasksUsedMonth,
        tasksLimitMonth: companies.tasksLimitMonth,

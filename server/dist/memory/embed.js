...
export async function embedText(text) {
    ...
    if (!response.ok) {
        ...
        throw new EmbedError(`Mistral Embed failed: ${response.status} ${response.statusText} — ${body}`);
    }
    const data = (await response.json());
    const embedding = data.data?.[0]?.embedding;
    if (!Array.isArray(embedding) || embedding.length !== EMBED_DIMS) {
        throw new EmbedError(`Unexpected embedding shape: expected ${EMBED_DIMS} dims, got ${embedding?.length ?? 0}`);
    }
    return embedding;
}

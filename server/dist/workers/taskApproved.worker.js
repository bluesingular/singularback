...
export const taskApprovedWorker = new Worker("agents", async (job) => {
    if (job.name !== "task.approved")
        return;
...
taskApprovedWorker.on("failed", (job, err) => {
    logger.error({ taskId: job?.data?.taskId, agentId: job?.data?.agentId, err }, "task.approved job failed");
});
taskApprovedWorker.on("error", (err) => {
    logger.error({ err }, "task-approved worker connection error");
});
//# sourceMappingURL=taskApproved.worker.js.map
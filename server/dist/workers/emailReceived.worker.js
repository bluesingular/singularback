...
export const emailWorker = new Worker("agents", async (job) => {
    if (job.name !== "email.received")
        return;
...
emailWorker.on("failed", (job, err) => {
    logger.error({ agentId: job?.data?.agentId, emailId: job?.data?.emailId, err }, "email.received job failed");
});
emailWorker.on("error", (err) => {
    logger.error({ err }, "email worker connection error");
});
//# sourceMappingURL=emailReceived.worker.js.map
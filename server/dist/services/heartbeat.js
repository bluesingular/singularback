...
const ACTIVE_HEARTBEAT_RUN_STATUSES = ["queued", "running"];
    ...
    async function hasActiveExecutionPath(companyId, issueId) {
        const [run, deferredWake] = await Promise.all([
            db
                ...
                .from(heartbeatRuns)
                .where(and(eq(heartbeatRuns.companyId, companyId), inArray(heartbeatRuns.status, [...ACTIVE_HEARTBEAT_RUN_STATUSES]), sql `${heartbeatRuns.contextSnapshot} ->> 'issueId' = ${issueId}`))
                .limit(1)
                .then((rows) => rows[0] ?? null),
            db
        ...
        try {
            ...
            try {
                ...
                const usageJson = normalizedUsage || adapterResult.costUsd != null
                    ...
                    : null;
                const persistedResultJson = mergeHeartbeatRunResultJson(adapterResult.resultJson ?? null, adapterResult.summary ?? null);

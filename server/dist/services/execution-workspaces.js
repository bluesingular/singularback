...
export function readExecutionWorkspaceConfig(metadata) {
    ...
    const config = {
        ...
        serviceStates: isRecord(raw.serviceStates)
            ? Object.fromEntries(Object.entries(raw.serviceStates).filter(([, state]) => state === "running" || state === "stopped"))
            : null,
    };
...
export function mergeExecutionWorkspaceConfig(metadata, patch) {
    ...
    const nextConfig = {
        ...
        serviceStates: patch.serviceStates !== undefined && isRecord(patch.serviceStates)
            ? Object.fromEntries(Object.entries(patch.serviceStates).filter(([, state]) => state === "running" || state === "stopped"))

...
export function packInstallService(db, agentQueue, systemQueue) {
    /**
     * Load a pack manifest from disk
     */
    async function loadPackManifest(packSlug) {
        ...
        packData.agents = (packData.agents || []).map((agent) => ({
            slug: agent.slug,
            name: agent.name,
        ...
        try {
            ...
            packData.qualityGates = (qualityGatesData.gates || []).map((gate) => {
                let gateType = "custom";
                if (gate.id?.includes("batch") || gate.id?.includes("volume"))
                    gateType = "volume_limit";
                else if (gate.id?.includes("whitelist") || gate.id?.includes("contact"))
                    gateType = "recipient_whitelist";
                else if (gate.id?.includes("budget"))
                    gateType = "budget_limit";
                else if (gate.id?.includes("tone") || gate.id?.includes("content") || gate.id?.includes("legal") || gate.id?.includes("gdpr"))
        ...
        try {
            ...
            packData.seedTasks = (seedTasksData.seed_tasks || []).map((task) => ({
        ...
        try {
            ...
            packData.activationSequence = (activationData.moments || []).map((moment) => ({
                key: moment.key || "",
     ...
     */
    async function install(companyId, packSlug, variables) {
...
//# sourceMappingURL=pack-install-service.js.map
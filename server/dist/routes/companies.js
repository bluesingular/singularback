...
import { accessService, agentService, budgetService, companyPortabilityService, companyService, feedbackService, logActivity, packInstallService, } from "../services/index.js";
...
export function companyRoutes(db, storage) {
    ...
    router.get("/:companyId/packs", async (req, res) => {
        ...
        try {
            const availablePacks = await packs.listAvailablePacks();
    ...
    router.post("/:companyId/packs/:packSlug/install", async (req, res) => {
        assertCompanyAccess(req, req.params.companyId);
        const companyId = req.params.companyId;
        const packSlug = req.params.packSlug;
        try {

...
import { getConsoleContext, approveConsoleCard, ConsoleApprovalError, } from "../console/service.js";
import { agentQueue } from "../queue/queues.js";
import { emit } from "../queue/emit.js";
export function consoleRoutes(db) {
    const router = Router();
    ...
    router.get("/companies/:companyId/console", async (req, res) => {
        ...
        try {
            const context = await getConsoleContext(db, agentQueue, companyId);
            res.json(context);
        }
        catch (err) {

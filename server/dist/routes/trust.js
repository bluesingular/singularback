...
export function trustRoutes(db) {
    ...
    router.get("/companies/:companyId/trust", async (req, res) => {
        ...
        const [scores, proposals] = await Promise.all([
            ...
            db
                .select()
                .from(trustProposals)
                .where(and(eq(trustProposals.companyId, companyId), eq(trustProposals.status, "pending"))),
        ]);
        res.json({ scores, proposals });
    });
    // POST /companies/:companyId/trust/proposals/:proposalId/approve
    router.post("/companies/:companyId/trust/proposals/:proposalId/approve", async (req, res) => {
        const { companyId, proposalId } = req.params;
        assertCompanyAccess(req, companyId);
        const [proposal] = await db
            .select()
            .from(trustProposals)
            .where(and(eq(trustProposals.id, proposalId), eq(trustProposals.companyId, companyId), eq(trustProposals.status, "pending")))
            .limit(1);
        if (!proposal) {
        ...
        await Promise.all([
            db
                ...
                .where(and(eq(trustScores.agentId, proposal.agentId), eq(trustScores.skillType, proposal.skillType))),
            db
    ...
    router.post("/companies/:companyId/trust/proposals/:proposalId/reject", async (req, res) => {
        const { companyId, proposalId } = req.params;
        assertCompanyAccess(req, companyId);

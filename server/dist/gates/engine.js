...
import { qualityGates, gateViolations, auditEntries, agents, } from "@paperclipai/db";
// ── Gate engine ───────────────────────────────────────────────────────────────
/**
 ...
 */
export async function runGates(db, params) {
...
// ── Gate checkers ─────────────────────────────────────────────────────────────
async function checkGate(db, gate, data) {
    switch (gate.gateType) {
        case "volume_limit": {
            const { maxPerDay } = gate.config;
            const todayCount = await getAgentActionCount(db, gate.agentId, "send_email");
        ...
        case "budget_limit": {
            const [agent] = await db
                ...
                .limit(1);
            if (!agent)
                return { passed: true }; // agent not found → don't block
        ...
        case "recipient_whitelist": {
            const { allowedDomains } = gate.config;
...
// ── DB helpers ────────────────────────────────────────────────────────────────
async function getActiveGates(db, companyId, agentId) {
    // Load gates that apply to this company+agent (agent-specific OR company-wide)
    const rows = await db
        .select()
        .from(qualityGates)
        .where(and(eq(qualityGates.companyId, companyId), eq(qualityGates.enabled, true)));
    // Include global (agentId=null) + agent-specific gates
    return rows.filter((g) => g.agentId === null || g.agentId === agentId);
}
async function getAgentActionCount(db, agentId, actionType) {
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const [row] = await db
        .select({ count: sql `count(*)::int` })
        .from(auditEntries)
        .where(and(eq(auditEntries.agentId, agentId), eq(auditEntries.actionType, actionType), eq(auditEntries.result, "success"), gte(auditEntries.createdAt, todayStart)));
    return row?.count ?? 0;
}
 ...
 */
async function createApprovalRequestStub(params) {
    void params;

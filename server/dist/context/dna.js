 ...
 */
export async function getDna(db, companyId) {
    const [row] = await db
        .select()
        .from(companyDna)
 ...
 */
export async function getDnaCompressed(db, companyId) {
    const dna = await getDna(db, companyId);
    if (!dna)
        return "";
    const parts = [];
    if (dna.description)
        parts.push(`Company: ${dna.description}`);
    if (dna.customerProfile)
        parts.push(`Customers: ${dna.customerProfile}`);
    if (dna.tone)
        parts.push(`Tone: ${dna.tone}`);
 ...
 */
export async function getDnaFull(db, companyId) {
    const dna = await getDna(db, companyId);
    if (!dna)
        return "";
    const sections = ["## Company Identity"];
    if (dna.description)
        sections.push(`**Description:** ${dna.description}`);
    if (dna.customerProfile)
        sections.push(`**Target customers:** ${dna.customerProfile}`);
    if (dna.tone)
        sections.push(`**Communication tone:** ${dna.tone}`);
    if (dna.brandRules) {
        sections.push("\n## Brand Rules");
 ...
 */
export async function upsertDna(db, companyId, data, updatedBy) {
    await db
        .insert(companyDna)
        .values({
        companyId,

...
function evaluateCondition(condition, rating) {
    if (condition === "cv_score_gte_4")
        return rating >= 4;
    return false;
}
// ── Main evaluator ────────────────────────────────────────────────────────────
export async function evaluateHandoffs(db, ctx) {
    ...
    for (const rule of handoffs) {
        if (!evaluateCondition(rule.condition, rating))
            continue;
        // Find the target agent by name in this company
        const [targetAgent] = await db
            .select({ id: agents.id })
            .from(agents)
            .where(and(eq(agents.companyId, companyId), eq(agents.name, rule.targetAgent)))
            .limit(1);
        if (!targetAgent) {
            logger.warn({ targetAgentName: rule.targetAgent, companyId }, "handoff: target agent not found");
            continue;
        }
        // Interpolate template variables
        const title = rule.taskTemplate.replace(/\{\{(\w+)\}\}/g, (_, key) => variables[key] ?? `{${key}}`);

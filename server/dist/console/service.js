 ...
 */
export async function getConsoleContext(db, agentQueue, companyId) {
    ...
    logger.info({ companyId, cardCount: cards.length, queueDepth, agentCount: trustState.length }, "console-service: context loaded");
 ...
 */
export async function approveConsoleCard(db, emitTaskApproved, params) {
    ...
    if (!updated || updated.length === 0) {
        throw new ConsoleApprovalError(`Card ${cardId} not found for company ${companyId}`);
    }

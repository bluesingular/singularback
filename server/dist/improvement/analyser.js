 ...
 */
export function shouldTriggerImprovement(trustScore, triggerReason) {
 ...
 */
export async function createDraftVersion(db, params) {
    const rows = await db
 ...
 */
export async function getActiveVersion(db, companyId, skillType) {
    const [row] = await db
        .select({
        id: skillVersions.id,
 ...
 */
export function bumpMinorVersion(current) {

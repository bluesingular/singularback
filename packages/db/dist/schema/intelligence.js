...
// ── intelligence_cards ────────────────────────────────────────────────────────
export const intelligenceCards = pgTable("intelligence_cards", {
    ...
    insightKeyIdx: index("intelligence_cards_insight_key_idx").on(t.companyId, t.insightKey),
}));
// ── activation_moments ────────────────────────────────────────────────────────
export const activationMoments = pgTable("activation_moments", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id")
        .notNull()
        .references(() => companies.id, { onDelete: "cascade" }),
    ...
    companyPackIdx: index("activation_moments_company_pack_idx").on(t.companyId, t.packSlug),
    // Each trigger fires at most once per company+pack
    uniqueTrigger: uniqueIndex("activation_moments_unique_trigger").on(t.companyId, t.packSlug, t.triggerKey),
}));
